...
def _extract_date_from_text(text: str) -> Optional[str]:
    """从文本中提取日期，返回 YYYY-MM-DD 格式，找不到返回 None"""
    import re
    now_year = datetime.now().year
    patterns = [
        r'(\d{4})[年\-/](\d{1,2})[月\-/](\d{1,2})',   # 2025年11月30日 / 2025-11-30
        r'(\d{4})[年\-/](\d{1,2})月?',                  # 2025年11月
        r'(20\d{2})(\d{2})(\d{2})',                      # 20251130
    ]
    for pat in patterns:
        ...
        if m:
            try:
                ...
                if 2000 <= year <= now_year + 1 and 1 <= month <= 12 and 1 <= day <= 31:
...
def _is_stale(date_str: str, max_days: int = 1) -> bool:
    """判断日期是否超过 max_days 天"""
    if not date_str:
        return False
    try:
        pub = datetime.strptime(date_str[:10], "%Y-%m-%d")
        return (datetime.now() - pub).days > max_days
    except Exception:
        return False
...
def _fetch_single_point(api_key: str, query: str, dimension: str, today: str) -> dict:
    ...
    cutoff_date = (datetime.now() - timedelta(hours=24)).strftime("%Y-%m-%d %H:%M")

    system = (
        "你是一名联网资讯助理。只输出JSON，不要其他内容。"
        f"今天是{today}。"
        f"严格限制：只能返回{cutoff_date}之后发布的内容，超过24小时的文章一律不使用。"
        f"{AUTHORITY_SOURCES}"
    )
    user = f"""
搜索：{query}
...
输出格式（严格JSON）：
{{"text": "一句话要点，信息密度高，50字以内", "sourceUrl": "原文URL", "sourceName": "来源名称（如：ESPN、虎扑）", "imageUrl": "文章封面图URL，没有则null", "publishDate": "文章发布日期YYYY-MM-DD，必填"}}

硬性要求：
- 只使用{cutoff_date}之后发布的文章，禁止使用更早的内容
- publishDate：文章实际发布日期，必须填写，不确定则填今天{today}
- 如果24小时内找不到相关内容，text填"（近期暂无相关资讯）"，publishDate填{today}
""".strip()
    ...
    try:
        ...
        # 校验1：publishDate 字段
        if publish_date and _is_stale(publish_date, max_days=1):
            print(f"[main] 丢弃过期内容(publishDate={publish_date}): {text[:40]}", flush=True)
            return {"text": "（近期暂无相关资讯）", "imageUrl": None, "sourceUrl": None, "sourceName": None}

        # 校验2：从 text 里抽取日期，二次确认
        text_date = _extract_date_from_text(text)
        if text_date and _is_stale(text_date, max_days=1):
            print(f"[main] 丢弃过期内容(text日期={text_date}): {text[:40]}", flush=True)
            return {"text": "（近期暂无相关资讯）", "imageUrl": None, "sourceUrl": None, "sourceName": None}

        # 校验3：URL 里包含明显过期的年月
        if source_url:
            url_date = _extract_date_from_text(source_url)
            if url_date and _is_stale(url_date, max_days=1):
                print(f"[main] 丢弃过期内容(URL日期={url_date}): {source_url[:60]}", flush=True)
                source_url = None  # 只清空 URL，保留文本（可能还有价值）
